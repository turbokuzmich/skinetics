# Phase 02 — Cream launch

- **State:** Not started
- **Owner:** Unassigned
- **Started:** —
- **Completed:** —

## Objective

Publish accurate, useful, indexable pages for the two priority face creams with separate Wildberries and Ozon purchase paths.

## Prerequisites

- [ ] Phase 01 complete
- [ ] Cream brands and product identities accepted
- [ ] Packaging, INCI, application, precautions, and approved images verified
- [ ] Cosmetic claims reviewed

## In scope

- two product records and product pages;
- verified product copy and images;
- Wildberries and Ozon actions;
- catalog inclusion, metadata, canonical URLs, sitemap, and internal links;
- product and marketplace analytics;
- responsive and content validation.

## Out of scope

- complete brand hub implementation;
- broad facial-care content clusters;
- final portfolio redesign;
- marketplace price synchronization.

## Deliverables

- published Neon Beard Ultra Lift, 100 ml, product page;
- published SkineticsLab ReneWal, 50 ml, product page with qualified cosmetic wording;
- updated catalog and relevant listings;
- search and analytics configuration for both products;
- launch validation evidence.

## Exit criteria

- [ ] Both pages use authoritative product data.
- [ ] Both marketplace pairings and outbound URLs are correct.
- [ ] Brand presentation matches the accepted brand decision.
- [ ] Product copy contains no unsupported medical or guaranteed claims.
- [ ] Pages are linked, indexable, present in the sitemap, and have unique metadata.
- [ ] Product and marketplace events are verified.
- [ ] Lint, build, and manual route checks pass.

## Related records

- [Roadmap](../ROADMAP.md#phase-02--cream-launch)
- [Product source of truth](../shared/product-source-of-truth.md)
- [Claims policy](../shared/claims-policy.md)
